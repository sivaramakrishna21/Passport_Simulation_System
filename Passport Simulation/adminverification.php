...
{
echo "<center><caption>Received details</caption></center>";
...
if( isset( $_REQUEST[$cars[$i]] )){
	...
	$sql1="update appstatus set adminstatus=1 where registerid='$cars[$i]'";
	mysqli_query($conn1, $sql1);
}
else if(isset( $_REQUEST[$cars[$i].$s2] ))
{
	$conn2=new mysqli("localhost","root","","sivaram");
	$sql2="update appstatus set adminstatus=-1,status=-1 where registerid='$cars[$i]'";	
